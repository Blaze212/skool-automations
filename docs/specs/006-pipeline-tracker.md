...
| Last touch         | Plugin / Skool      | Updated by plugin on every connection or DM event; stored as `M/D/YYYY h:mm:ss AM/PM` |
...
`Date connected` and `Date first DM sent` are written as `YYYY-MM-DD` strings. `Last touch` is
written as `M/D/YYYY h:mm:ss AM/PM` (e.g. `5/28/2026 2:34:09 PM`) using the full event timestamp,
so the cell is recognized as a datetime value by Google Sheets.
