...
function makeMockClient(insertResult = { data: { id: 'row-uuid' }, error: null }) {
  ...
  return { client: { from } as ReturnType<typeof from>, mocks: { from, insert, update, select, single, eq } }
}
...
describe('FractionalDb', () => {
  describe('insertClient', () => {
    ...
    it('throws when the DB returns an error', async () => {
      ...
      await expect(
        db.insertClient({ full_name: 'Jane', drive_email: 'j@e.com', skool_email: 'j@e.com', program_start_date: null, notes: null }),
