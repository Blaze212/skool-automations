...
function parseTabBody(
  ...
  let current:
    | (Omit<DocSection, 'headingId' | 'anchorLink'> & {
...
async function main() {
  ...
  if (!docId) {
    console.error('Missing: GOOGLE_DOC_WORKBOOK_ID');
    process.exit(1);
  }
  if (!supabaseKey) {
  ...
  if (fetchErr) {
    ...
    console.error(
      `  Key:    ${supabaseKey.slice(0, 12)}...${supabaseKey.slice(-4)} (length ${supabaseKey.length})`,
    );
    if (fetchErr.code === 'PGRST301' || /No suitable key/.test(fetchErr.message)) {
      ...
      console.error(
        '  Run `supabase status` and use the "Secret" value as SUPABASE_SERVICE_ROLE_KEY.',
      );
  ...
  const existingMap = new Map<string, { hash: string; model: string | null }>(
    (existing ?? []).map((r: any) => [
      r.heading_id,
      { hash: r.content_hash, model: r.embed_model },
    ]),
  );
  ...
  if (stale.length > 0) {
    console.log(
      `Detected ${stale.length} chunk(s) embedded with a different model. Wiping for re-embed...`,
    );
    const { error: delErr } = await supabase
      .from('workbook_chunks')
      .delete()
      .neq('heading_id', '__never__');
    if (delErr) {
