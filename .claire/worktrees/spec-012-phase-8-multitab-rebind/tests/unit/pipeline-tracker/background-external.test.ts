...
afterEach(() => {
  // no module-level state to reset in background-external.ts
});
...
describe('handleExternalMessage — ping', () => {
  ...
  it('confirmed + wrong token → {installed: true, bound: false} (no version)', async () => {
    installStatefulStorage({
      [STORAGE_KEYS.BINDING]: makeBinding(),
    });
    const result = await handleExternalMessage(
    ...
    // Version is NOT revealed to a caller that doesn't hold the token.
...
describe('handleExternalMessage — sync-pull', () => {
  ...
  it('confirmed + correct token → rows and syncedIds match outbox entries', async () => {
    ...
    // Row events are copies of the OutboxEntry.event — verify one field.
  ...
  it('only attaches recovered_html to the rows that have it', async () => {
    installStatefulStorage({
      [STORAGE_KEYS.BINDING]: makeBinding(),
      [STORAGE_KEYS.OUTBOX]: [makeOutboxEntry('h-1'), makeOutboxEntry('h-2')],
      'recovered_html_h-1': '<div>html for h-1</div>',
      // h-2 has no recovered_html
    });
    const result = (await handleExternalMessage(
      { type: 'sync-pull', bindingToken: 'test-token-abc' },
      validSender,
  ...
  it('idempotent — second call returns same rows without mutation', async () => {
    ...
    // Outbox is unchanged — no writes during sync-pull.
  ...
  it('excludes already-acked entries (entries removed from outbox by sync-ack)', async () => {
    const local = installStatefulStorage({
      [STORAGE_KEYS.BINDING]: makeBinding(),
      [STORAGE_KEYS.OUTBOX]: [makeOutboxEntry('h-1'), makeOutboxEntry('h-2')],
    });
    ...
    // Second pull — h-1 is gone; h-2 is still present.
